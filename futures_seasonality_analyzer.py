...
class FuturesSeasonalityAnalyzer:
    ...
    def generate_comprehensive_report2(self, seasonal_stats, returns_stats, monthly_stats, oi_stats, merged_data, merged_oi, oi_positions):
        """
        生成综合季节性分析报告
        """
        print("=" * 80)
        ...
        if len(current_data) > 0:
            ...
            for _, row in current_data.iterrows():
                if 'close_y' in row and not pd.isna(row['close_y']) and row['max'] > row['min']:
                    ...
                    current_prices.append(row['close_y'])
            
            if current_position:
                avg_position = np.mean(current_position)
                position_level = "🔴 历史高位" if avg_position > 0.7 else "🟢 历史低位" if avg_position < 0.3 else "🟡 历史中位"
                ...
                # 分析价格分布
                high_days = sum(1 for p in current_position if p > 0.7)
                low_days = sum(1 for p in current_position if p < 0.3)
                mid_days = len(current_position) - high_days - low_days
                
                print(f"   位置分布: 高位{high_days}天, 中位{mid_days}天, 低位{low_days}天")
                
                # 近期趋势
                if len(current_position) >= 10:
                    recent_trend = "上升" if current_position[-1] > current_position[-10] else "下降"
                    print(f"   近期趋势: {recent_trend}")
        ...
        # 检查持仓量数据可用性
        if merged_oi is not None:
            current_oi_cols = [col for col in merged_oi.columns if 'open_interest' in col and ('_y' in col or col == 'open_interest')]
            if current_oi_cols:
                current_oi_col = current_oi_cols[0]
                current_oi_data = merged_oi[current_oi_col].dropna()
                print(f"   当前年份有效数据点: {len(current_oi_data)}个")
        ...
        if valid_positions:
            ...
            if extreme_high_count > 0:
                print(f"   ⚠️  持仓量极端高位: {extreme_high_count}天 (>80%)")
            if high_count > 0:
                print(f"   🔴 持仓量高位: {high_count}天 (>70%)")
            if low_count > 0:
                print(f"   🟢 持仓量低位: {low_count}天 (<30%)")
            if extreme_low_count > 0:
                print(f"   ⚠️  持仓量极端低位: {extreme_low_count}天 (<20%)")
                
            # 持仓量集中度分析
            if len(valid_positions) > 0:
                position_std = np.std(valid_positions)
                concentration = "集中" if position_std < 0.2 else "分散" if position_std > 0.3 else "适中"
                print(f"   持仓量分布: {concentration} (标准差: {position_std:.3f})")
        else:
            print("   ℹ️  当前年份持仓量数据不足或无法计算位置")
            if oi_stats is not None:
                print(f"   历史持仓量数据范围: {oi_stats['min'].min():.0f} - {oi_stats['max'].max():.0f}")
        ...
        # 投资建议
        print(f"\n💎 季节性投资建议:")
        if current_month in monthly_stats['month'].values:
            ...
            total_score = return_score + win_score + volatility_score
            
            if total_score >= 3:
                recommendation = "🟢 强烈看涨"
                reasoning = "历史表现优秀，季节性支撑强劲，胜率较高"
            elif total_score >= 2:
                recommendation = "🔵 温和看涨" 
                reasoning = "季节性因素偏正面，表现稳定"
            elif total_score >= 1:
                recommendation = "🟡 中性"
                reasoning = "季节性信号不明显，建议结合其他分析"
            elif total_score >= 0:
                recommendation = "🟠 温和看跌"
                reasoning = "季节性因素偏负面，需谨慎操作"
            else:
                recommendation = "🔴 强烈看跌"
                reasoning = "历史表现疲弱，季节性压力明显"
            ...
            # 结合持仓量信号
            if valid_positions and avg_position > 0.7:
                print(f"      • 持仓量处于历史高位，注意风险")
            elif valid_positions and avg_position < 0.3:
                print(f"      • 持仓量处于历史低位，可能存在机会")
        
        # 季节性策略总结
        print(f"\n📋 季节性策略总结:")
        print(f"   1. 关注最佳月份: 1月、6月、7月")
        print(f"   2. 避开弱势月份: 9月、10月、3月") 
        print(f"   3. 当前月份信号: {'正面' if total_score >= 2 else '中性' if total_score >= 1 else '负面'}")
        print(f"   4. 数据可靠性: {reliability if 'reliability' in locals() else '中等'}")
